<template>
	<view class="u-countdown">
		<view class="u-countdown-item" :style="[itemStyle]" v-if="showDays && (hideZeroDay || (!hideZeroDay && d != '00'))">
			<view :class="[isPadding?'u-countdown-time time-padding-all':'u-countdown-time time-padding-td']" :style="[letterStyle]">
				{{ d }}
			</view>
		</view>
		<view
			:class="[isPadding?'u-countdown-colon padding-all':'u-countdown-colon padding-td']"
			:style="{fontSize: separatorSize + 'rpx', color: separatorColor, paddingBottom: separator == 'colon' ? '8rpx' : 0}"
			v-if="showDays && (hideZeroDay || (!hideZeroDay && d != '00'))"
		>
			{{ format.length !== 0 ? format[0] :separator == 'colon' ? ':' : '天' }}
		</view>
		<view class="u-countdown-item" :style="[itemStyle]" v-if="showHours">
			<view :class="[isPadding?'u-countdown-time time-padding-all':'u-countdown-time time-padding-td']" :style="{ fontSize: fontSize + 'rpx', color: color}">
				{{ h }}
			</view>
		</view>
		<view
			:class="[isPadding?'u-countdown-colon padding-all':'u-countdown-colon padding-td']"
			:style="{fontSize: separatorSize + 'rpx', color: separatorColor, paddingBottom: separator == 'colon' ? '12rpx' : 0}"
			v-if="showHours"
		>
			{{ format.length !== 0 ? format[1] :separator == 'colon' ? ':' : '时' }}
		</view>
		...
		<view
			...
			v-if="showMinutes"
		>
		{{ format.length !== 0 ? format[2] :separator == 'colon' ? ':' : '分' }}
		</view>
